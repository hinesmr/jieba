...
def gen_pfdict(f_name):
        ...
        while True :
            ...
            try:
                ...
                if not use_sqlite :
                    ...
                    use_sqlite["conn"].execute("insert into FREQ_prenormalized values (?, ?)", (word, freq))
                ltotal += freq
                for ch in xrange(len(word)):
                    if not use_sqlite :
                        pfdict.add(word[:ch+1])
                    else :
                        use_sqlite["conn"].execute("insert into pfdict values (?)", (word[:ch+1],))
    ...
    if not use_sqlite :
        ...
        # To normalize with sqlite and avoid too much memory usage, 
        # we have to pass over the whole table again and write to a new table.
        # Then drop the non-normalized table when we are finished.

        logger.debug("Normalizing sqlite cache...")
        ...
        while True :
            ...
            use_sqlite["conn"].execute("insert into FREQ values (?, ?)", (word, norm_freq))
            if not gen_min_freq or norm_freq < gen_min_freq :
                gen_min_freq = norm_freq

        use_sqlite["conn"].execute("drop table FREQ_prenormalized")
...
def init_sqlite(cache_file) :
    ...
    conn.execute("create table if not exists FREQ_prenormalized (word text, freq real)")
